...
import ModelSelector from "../ui/select-model";
...
const ChatWindow: React.FC<ChatWindowProps> = ({ sessionId }) => {
  ...
  const { user } = useAuth();
  const { currentSession, startSession, endSession, logActivity } =
    useSession();
  ...
  const loadExistingConversations = async (sessionId: string) => {
    ...
    try {
      ...
      if (response?.success && response.conversations?.length > 0) {
        const existingMessages = convertConversationsToMessages(
        ...
        console.log(
          "No conversations found or unexpected response structure:",
          response
        );
        setConversationsLoaded(true);
      }
  ...
  const handleMicToggle = async () => {
    ...
    // Check if speech recognition failed due to unsupported browser
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      isSubmitting ||
      !currentSession ||
      cleanedTranscript === lastProcessedTranscript
    ) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        await logActivity("error_occurred", {
          context: "api_call",
          error:
            apiError instanceof Error ? apiError.toString() : String(apiError),
        });
      }
    } catch (error) {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="border-b px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button variant="ghost" size="icon" className="mr-[10px]">
             <ModelSelector/>
            </Button>
          </div>
        </div>
              ...
              {isLoadingConversations ? (
                ...
                <>
                  <div className="space-y-4">
                    ...
                    <p className="text-muted-foreground text-lg">
                      Press the microphone button to start your conversation
        ...
        {chatStarted && (
          <>
            ...
            <div
              className={cn(
                "relative overflow-hidden transition-all duration-500 ease-in-out flex items-center justify-center bg-black",
                    ...
                    >
                      {message.sender === "ai" && (
                        <div className="mr-2">
                          <CustomAvatar
                            isAI
                            animationState={aiAvatar.animationState}
                          />
                        </div>
                      )}
                      ...
                      {message.sender === "user" && (
                        <div className="ml-2">
                          <CustomAvatar
                            user={user}
                            animationState={userAvatar.animationState}
                          />
                        </div>
                      )}
                    </div>
              ...
              <div className="max-w-4xl mx-auto">
                {!speechSupported && (
                  <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-yellow-800 text-sm">
                      ...
                      Please use Chrome on desktop or try typing your message
                      above.
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center space-x-3">
                  <Button
                    ...
                    onClick={handleMicToggle}
                    disabled={isUploading || !speechSupported}
                    aria-label={
                  ...
                  {/* Rest of the input area remains the same */}
                  ...
                  <Button
                    ...
                    disabled={
                      !displayTranscript.trim() || isSubmitting || isUploading
                    }
